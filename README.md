...
3. Frontend запрашивает у Backend полные детали прогноза и итоговую оценку риска.
4. Backend возвращает:
   - данные пациента;
   - рассчитанный прогноз;
   - список факторов риска;
   - категорию риска, перцентиль и текстовую рекомендацию.
5. Веб-интерфейс отображает единый отчёт анализа.
6. В отчёте аналитик видит:
   - итоговую стоимость;
   - ключевые факторы влияния на прогноз;
   - данные пациента;
   - итоговую категорию риска;
   - рекомендации по кейсу.
7. Из того же интерфейса аналитик может запустить перерасчёт или экспортировать отчёт в PDF.
...
6. При клике по строке таблицы frontend запрашивает детали выбранного прогноза.
7. Backend возвращает полную информацию по записи.
8. Frontend открывает страницу `/predict` в режиме готового отчёта анализа.
9. Аналитик может изучить сохранённый отчёт, экспортировать его в PDF или перейти к перерасчёту.
